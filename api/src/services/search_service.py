...
from ports.vector_store import CollectionNotFoundError, VectorStorePort
        ...
        for domain_id in domain_ids:
            try:
                ...
                for vr in vector_results:
                    ...
                    # Supported formats: {uuid}_{index} or {uuid}-chunk-{index}
                    chunk_id = vr.chunk_id
                    document_id_str = chunk_id
                    
                    if "_chunk_" in chunk_id:
                        document_id_str = chunk_id.split("_chunk_")[0]
                    elif "-chunk-" in chunk_id:
                        document_id_str = chunk_id.split("-chunk-")[0]
                    elif "_" in chunk_id:
                        document_id_str = chunk_id.rsplit('_', 1)[0]
                ...
                # Domain has no documents, so no collection exists in vector store
                # This is a valid state, just continue to the next domain
                continue
            except Exception as e:
                # Log other errors but continue with other domains
    ...
    async def keyword_search(
        ...
        sql = text("""
            ...
            WHERE
                ...
                AND LOWER(d.status::text) = 'done'
                AND to_tsvector('simple', COALESCE(d.title, ''))
                    @@ plainto_tsquery('simple', :query)
            ORDER BY rank DESC
            LIMIT :limit
        """)
