...
```bash
INIT=$(node "D:/02-PERSONAL/01-PROJECTS/25-KnowledgeManagement/.opencode/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "D:/02-PERSONAL/01-PROJECTS/25-KnowledgeManagement/.opencode/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```bash
node "D:/02-PERSONAL/01-PROJECTS/25-KnowledgeManagement/.opencode/get-shit-done/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```
